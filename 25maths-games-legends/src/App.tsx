...
export default function App() {
  ...
  const isAutotest = searchParams.get('autotest') === '1' || searchParams.get('demo') === '1';
  ...
  // Autotest helpers: select a default hero so Playwright can proceed with minimal clicks
  useEffect(() => {
    if (!isAutotest) return;
    if (selectedCharId) return;
    if (!CHARACTERS[0]?.id) return;
    setSelectedCharId(CHARACTERS[0].id);
  }, [isAutotest, selectedCharId]);

  // Autotest helpers: pick a default grade to enter the map screen
  useEffect(() => {
    if (!isAutotest) return;
    if (gameState !== 'map') return;
    if (!profile) return;
    if (profile.grade) return;
    updateProfile({ grade: 7 });
  }, [isAutotest, gameState, profile, updateProfile]);
  ...
  const handleGuest = () => {
    ...
    if (isAutotest) setGameState('map');
  };

  const isLoggedIn = !!user || isGuest;
  const isAdmin = user?.email === 'zhuxingda86@hotmail.com';

  // Expose minimal state for the Playwright harness (render_game_to_text reads this)
  useEffect(() => {
    const activeRoomPlayers = activeRoom ? Object.keys(activeRoom.players || {}).length : 0;
    (window as any).__gl_game_state = {
      mode: gameState,
      lang,
      isGuest,
      isLoggedIn,
      ...
      profile: profile
        ? { grade: profile.grade ?? null, total_score: profile.total_score ?? 0, display_name: profile.display_name ?? null }
        : null,
      selectedCharId: selectedCharId || profile?.selected_char_id || null,
      activeMission: activeMission ? { id: activeMission.id, grade: activeMission.grade, unitId: activeMission.unitId, order: activeMission.order } : null,
      overlays: { showSecret, showSkillCards, selectedSkillCard },
      daily: { isDailyBattle, multiplier: isDailyBattle ? DAILY_MULTIPLIER : 1 },
      multiplayer: activeRoom ? { id: activeRoom.id, status: activeRoom.status, players: activeRoomPlayers } : null,
      coord: 'UI-based. Origin top-left; x→right; y→down.',
    };
  }, [
    activeMission,
    activeRoom,
